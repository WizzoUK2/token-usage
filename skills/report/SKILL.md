...
- **Activity** — a slash command (one row per command name, summed across invocations) or `(no command)` for turns before the first command in the session. `(+N agents)` means N subagent transcripts were rolled up into that row.
